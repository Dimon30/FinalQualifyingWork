...
- **4 архитектуры V*-оптимизатора**: MLP, SAC, TD3, PPO — переобучены на dataset_v2.csv (~3k строк, после drag-фикса) ✓
...
- **Полный бенчмарк** (4 модели × 4 кривые, 2026-05-18 после регуляризации MLP):

| Модель | spiral_r3 | circle_r3z5 | helix_r2 | line_diag |
|---|---|---|---|---|
| MLP | 0.026 / 2.11× ✓ | 0.012 / 2.51× ✓ | **0.016 / 1.84× ✓** | 0.398 / 1.92× |
| SAC | 0.015 / 1.11× ✓ | 0.025 / 1.28× ✓ | 0.009 / 1.00× ✓ | 0.021 / 1.10× ✓ |
| TD3 | 0.010 / 1.18× ✓ | 0.009 / 1.72× ✓ | 0.011 / 1.03× ✓ | 0.042 / 1.17× ✓ |
| PPO | 0.041 / 1.61× ✓ | 0.014 / 2.26× ✓ | 0.057 / 1.42× ✓ | 0.040 / 1.69× ✓ |

Значения: e2_rms (м) / speedup (×). Порог сходимости e2_rms<0.1.

**Главное достижение**: SAC/TD3/PPO стабильны на всех 4 кривых; MLP сходится на 3/4 после регуляризации (helix_r2: 18.9 → 0.016, в ~1180 раз). На line_diag MLP остаётся хуже остальных (0.4), но дрон не улетает — поведение ограниченное.
...
- **Текущий датасет**: `dataset_v2.csv` — 8 процессов × ~380 строк ≈ **~3,070 строк** (2026-05-17, после drag-фикса). Параметры: `--curves 200 --samples 8 --oracle-horizon 1500 --oracle-speed-step 1.0 --seeds 1..8`. Старый `dataset_large.csv` (41k строк, без drag) сохранён как референс.
- **drag = 0.05** в `QuadModel` — реалистичный квадратичный drag в `quad_dynamics_16`. Терминальная скорость ≈ 12 м/с при наклоне 30°. **Решило проблему разгона ‖v‖ до 200+ м/с.**
- **anti-windup для η откатан**: пробовали `freeze eta_dot при насыщении Ubar`, но это ломало регулятор Гл. 4 на тугих спиралях (диверж до ‖v‖=17 при V*=1). Drag сам по себе ограничивает разгон.
- **Углы x0 в `_make_state_on_curve`**: yaw=yaw_star(s), pitch=**0** (НЕ beta(s)). Pitch=beta вызывал диверж переходного процесса.
- **Шум в state**: pos_std=0.01, vel_std=0.02 (было 0.05 и 0.1). vel_std=0.1 при V_min=0.3 = 33% возмущение, разрушало переходный.
...
- **[ИСПРАВЛЕНО] MLP расходится на helix_r2** (2026-05-18, `ml/training/train_model.py` + `ml/models/speed_model.py`):
  - Причина: MLP без регуляризации (weight_decay=0, dropout=0) переуверенно экстраполировал V*=7.4 на спирали r=2,k=1, требующий тангаж 85° — физически невозможный.
  - Фикс: добавлены параметры `dropout` (опциональный nn.Dropout в SpeedMLP) и `weight_decay` в `Adam`. Default-значения после подбора: `dropout=0.0, weight_decay=5e-4`. Регуляризация перенесена в registry-формат чекпоинта (поле `dropout`), обратно-совместимо со старыми `.pt`.
  - Результат: helix_r2 e2_rms 18.92 → 0.016 (×1180), v_mean 7.42 → 1.84 м/с. spiral_r3/circle_r3z5 сохранили сходимость (e2_rms <0.03). line_diag деградировал 0.068→0.398 (всё ещё ограничен, не расходится в бесконечность).
- **[ИСПРАВЛЕНО] PPO offline R²≈−0.98 и коллапс к константе ~5.2** (2026-05-18, `ml/training/train_rl_models.py:train_ppo`):
  - Баг 1: `old_log_probs` индексировались по shuffled DataLoader → рассинхрон с батчами в мини-эпохах. Решено фиксированной permutation на эпоху и нарезкой по offset.
  - Баг 2: `adv = z-score(V_opt)` не зависит от действия → при `y_opt < mean(y)` отталкивает policy от целевого V_opt. Заменено на advantage от качества предсказания: `adv = -(mean.detach() - yb)² / max_speed²`.
  - Добавлен BC-якорь: `c_bc·MSE(mean, yb)` (c_bc=1.0). Surrogate стал мягким регуляризатором (c_policy=0.1).
  - NaN/Inf guard на policy output, ratio и loss; grad_clip 0.5 → 1.0.
  - Результат: val_MSE=0.32, R²=0.67, V_pred spread=6.3 м/с; на helix_r2 e2_rms=0.057 (было 11.5), на line_diag e2_rms=0.040 (было 0.399).
