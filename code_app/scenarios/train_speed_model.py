...
def plot_loss_curves(result: TrainResult, out_dir: str) -> None:
    ...
    ax = axes[0]
    ax.plot(epochs, result.train_losses, label="Обучение (MSE)", color="steelblue", linewidth=1.8)
    ax.plot(epochs, result.val_losses, label="Валидация (MSE)", color="coral", linewidth=1.8)
    ...
    ax = axes[1]
    ax.semilogy(epochs, result.train_losses, label="Обучение (MSE)", color="steelblue", linewidth=1.8)
    ax.semilogy(epochs, result.val_losses, label="Валидация (MSE)", color="coral", linewidth=1.8)
    ax.axvline(result.stopped_epoch, color="gray", linestyle="--", linewidth=1.2,
               label=f"Ранняя остановка (эпоха {result.stopped_epoch})")
    ax.set_xlabel("Эпоха")
    ax.set_ylabel("MSE (лог. шкала)")
    ax.set_title("Кривые потерь (логарифмическая шкала)")
    ax.legend()
    ax.grid(True, linestyle="--", alpha=0.6, which="both")

    fig.suptitle(f"Обучение SpeedMLP — лучшая валидационная MSE = {result.best_val_loss:.6f}", fontsize=12)
    fig.tight_layout()
