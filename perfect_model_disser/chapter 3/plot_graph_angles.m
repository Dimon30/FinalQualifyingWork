...
phi_values = squeeze(phi.signals.values);
psi_values = squeeze(psi.signals.values);
theta_values = squeeze(theta.signals.values);
...
% Настройка осей
axis([min(time) max(time) -1 1]); % Установите диапазон осей по необходимости
