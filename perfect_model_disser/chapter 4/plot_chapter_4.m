...
plot(time, X_err, 'LineWidth', 3, 'Color', [0.4660 0.6740 0.1880], 'LineStyle', '--', 'DisplayName', '$s$');
hold on; % Держим текущий график для добавления новых линий
plot(time, Y_err, 'LineWidth', 3, 'Color', [0.9290 0.6940 0.1250], 'LineStyle', '-.', 'DisplayName', '$e_1$');
plot(time, Z_err, 'LineWidth', 3, 'Color', [0 0.4470 0.7410], 'LineStyle', '-', 'DisplayName', '$e_2$');
...
% legend_output = legend('s', 'e_1', 'e_2', 'Location', 'NorthEast',  'FontSize', fs);
...
vel = squeeze(simOut.get('vel').Data);   % Значения vel
stil = squeeze(simOut.get('d').Data);   % Значения Y_err
% Z_err = squeeze(simOut.get('Z_err').Data);   % Значения Z_err
...
plot(time, vel, 'LineWidth', 2, 'Color', [0.0078, 0.4470, 0.7410], 'DisplayName', 'v');

legend_output = legend({'v, м/с', 'Location', 'NorthEast'});
set(legend_output, 'Interpreter', 'none', 'FontSize', fs); 

xlabel('$t\,(\mathrm{c})$', 'FontSize', fs, 'Interpreter', 'latex');
hold off;

% Создание графика s_til
figure;
set(gcf, 'Position', [250 + w/2, 250 + h/2, w/2, h/2]); % Исправлено на правильный синтаксис
hold on;
grid on;
...
plot(time, stil, 'LineWidth', 2, 'Color', [0.0078, 0.4470, 0.7410]);

legend_output = legend({'δ, рад', 'Location', 'NorthEast'});
set(legend_output, 'Interpreter', 'none', 'FontSize', fs); 

xlabel('$t\,(\mathrm{c})$', 'FontSize', fs, 'Interpreter', 'latex');
%ylabel('$\delta\,(\mathrm{m})$', 'FontSize', fs, 'Interpreter', 'latex');
hold off;
