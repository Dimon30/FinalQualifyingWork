...
Базовая непрерывная модель берется из диссертации~\cite{kim_algorithms}.
Вектор состояния расширенной модели имеет вид:
\begin{gather}
    x =
    \begin{bmatrix}
        p_x & p_y & p_z &
...
\end{gather}
Тогда уравнения для интеграторных расширений принимают вид
\begin{gather}
...
\end{gather}
где $s$ - параметр кривой. В проекте используются кривые с постоянной нормой касательного вектора, так как именно для этого класса корректно работает итоговый алгоритм диссертации~\cite{kim_algorithms}:
\begin{gather}
...
\begin{enumerate}
    \item контроллер гарантированно работает на кривых с постоянной нормой касательного вектора;
...
Таким образом, математическая модель задачи включает нелинейную динамику квадрокоптера, геометрию пространственной кривой, сопутствующую систему координат и наблюдатель параметра ближайшей точки. 
Именно в этой модели далее строится регулятор слежения по выходу и интеллектуальный модуль подбора скорости.